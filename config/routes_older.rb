ActionController::Routing::Routes.draw do |map|
  ...
  map.resources :feedbacks,               :only       => [:show, :index],
                                          :collection => {:deliver_feedback_form => :post,
                                                          :thank_you             => :get}
  map.resources :page_nodes,              :as         => "page", 
                                          :only       => [:show],
                                          :member     => {:document_audit_request => :post, 
                                                          :print_audit_request    => :post}
  ...
  map.namespace :admin do |admin|
    ...
    admin.resources :gallery_images,          :except     => [:show],
                                              :collection => {:order               => :post}
    admin.resources :galleries,               :except     => [:show],
                                              :collection => {:order               => :post}
    ...
    admin.resources :products,                :except     => [:show],
                                              :collection => {:index_update        => :post}
    admin.resources :surveys,                 :except     => [:show],
                                              :collection => {:order               => :post}
