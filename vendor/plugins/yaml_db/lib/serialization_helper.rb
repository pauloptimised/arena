module SerializationHelper
  ...
  class Base
    ...
    def super_dump(filename)
      disable_logger
      @dumper.super_dump(File.new(filename, "w"))
      reenable_logger
    end
  ...
  class Dump
    ...
    def self.super_dump(io)
    	super_tables.each do |table|
        before_table(io, table)
        dump_table(io, table)
        after_table(io, table)
      end
    ...
    def self.super_tables
    	ActiveRecord::Base.connection.tables.select{|table| ['features', 'sections', 'site_settings', 'roles', 'features_roles'].include?(table)}
    end
