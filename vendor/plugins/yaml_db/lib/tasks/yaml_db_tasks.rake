namespace :db do
	...
	namespace :super_data do
		def super_db_dump_data_file (extension = "yml")
		"#{dump_dir}/super_data.#{extension}"
    end
            
    def super_dump_dir(dir = "")
      "#{RAILS_ROOT}/db#{dir}"
    end
		...
		task :dump => :environment do
            format_class = ENV['class'] || "YamlDb::Helper"
            helper = format_class.constantize
			SerializationHelper::Base.new(helper).super_dump super_db_dump_data_file helper.extension
		end
		...
		task :load => :environment do
            format_class = ENV['class'] || "YamlDb::Helper"
            helper = format_class.constantize
			SerializationHelper::Base.new(helper).load (super_db_dump_data_file helper.extension)
		end
